...
CodeMirror.autoLoadMode = (instance, mode, resolve, reject) => {
  if (CodeMirror.modes.hasOwnProperty(mode)) {
...
const loadMode = (cm, mode) => new Promise((resolve, reject) => {
  if (mode) {
    console.log(`LOADING ${mode}`);
    CodeMirror.autoLoadMode(cm, mode, resolve, reject);
  } else {
    console.log('no mode to load');
    resolve('no mode to load');
  }
});
